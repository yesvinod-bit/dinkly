...
const PRIMARY_URL = 'https://dinkly-715753958407.us-west1.run.app';
...
function log(message) {
  console.log(`[deploy:run] ${message}`);
}
...
function buildArchive() {
  const workingDir = mkdtempSync(path.join(tmpdir(), 'dinkly-run-'));
...
function patchServiceBody(service, sourceObject) {
  const containers = service.template?.containers ?? [];
  const nextContainers = containers.map((container) => {
    if (container.name !== 'app-container') return container;
    return {
      ...container,
      sourceCode: {
        cloudStorageSource: {
...
async function main() {
  ...
  try {
    ...
    const body = patchServiceBody(service, uploaded);

    log('Starting Cloud Run rollout...');
...
main().catch((error) => {
  console.error(`[deploy:run] ${error.message}`);
  process.exit(1);
});
